using Biz.Lib.TACM.MauIn.Model;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace Biz.TACM.Models.ViewModel.MauIn
{
    public class MauInBiaHoSoViewModel
    {
        public string TenToaAn { get; set; }
        public string MaHoSo { get; set; }
        public string SoHoSo { get; set; }
        public string SoThuLy { get; set; }
        public string NhomAn { get; set; }
        public string LoaiQuanHe { get; set; }
        public string QuanHePhapLuat { get; set; }
        public string ThamPhanChuToa { get; set; }
        public string GioiTinhThamPhanChuToa { get; set; }
        public string ThamPhanKhac{ get; set; }
        public string GioiTinhThamPhanKhac { get; set; }
        public string ThamPhan1 { get; set; }
        public string GioiTinhThamPhan1 { get; set; }
        public string ThamPhan2 { get; set; }
        public string GioiTinhThamPhan2 { get; set; }
        public string HoiThamNhanDan1 { get; set; }
        public string GioiTinhHoiThamNhanDan1 { get; set; }
        public string HoiThamNhanDan2 { get; set; }
        public string GioiTinhHoiThamNhanDan2 { get; set; }
        public string HoiThamNhanDan3 { get; set; }
        public string GioiTinhHoiThamNhanDan3 { get; set; }
        public string ThuKy { get; set; }
        public string GioiTinhThuKy { get; set; }
        public string KiemSatVien { get; set; }
        public string GioiTinhKiemSatVien { get; set; }
        public DateTime NgayThuLy { get; set; }
        public DateTime NgayRaQuyetDinhXetXu { get; set; }
        public string TenCoQuanDeNghi { get; set; }
        public string BienPhapXuLyHanhChinh { get; set; }
        public List<DuongSuModel> DanhSachDuongSuAD { get; set; }
        public List<DuongSuModel> DanhSachNguyenDon { get; set; }
        public List<DuongSuModel> DanhSachBiDon { get; set; }
        public List<DuongSuModel> DanhSachNguoiLienQuanTranhChap { get; set; }
        public int GiaiDoan { get; set; }
        public int HoiDong { get; set; }
        public string SoQuyetDinh { get; set; }
        public DateTime NgayRaQuyetDinh { get; set; }
        public string NoiDungQuyetDinh { get; set; }
        public string SoBanAn { get; set; }
        public DateTime NgayRaBanAn { get; set; }
        public string NoiDungBanAn { get; set; }
        public int SoKhangNghi { get; set; }
        public DateTime NgayKhangNghi { get; set; }
        public int SoBiCan { get; set; }
        public string SoCaoTrang { get; set; }
        public DateTime NgayCaoTrang { get; set; }
        public List<DuongSuModel> DanhSachBiCao { get; set; }
        public List<NguoiKhangCaoGXNModel> DanhSachNguoiKhangCao { get; set; }
        public string ThuLyAD { get; set; }
        public DateTime NgayNopDonTaiToaAn { get; set; }
        public string SoBanAnPT { get; set; }
        public DateTime NgayRaBanAnPT { get; set; }
        public string SoQuyetDinhPT { get; set; }
        public DateTime NgayRaQuyetDinhPT { get; set; }
        public string VienKiemSatKhangNghi { get; set; }
        public string NoiDungKhangNghi { get; set; }
        public string NoiDungBanAnPT { get; set; }
        public string NoiDungQuyetDinhPT { get; set; }
        public DateTime HieuLuc { get; set; }
        public string ToaAnSoTham { get; set; }
    }
}